...
export default class VueI18NExtract {
  ...
  public async generateSchemaFile(report: I18NReport, writePath: string): Promise<NodeJS.ErrnoException | void> {
    if (!report.missingKeys) {
      // tslint:disable-next-line
      console.log('There is no Missing Files, skiping Generation...');
      return;
    }
    ...
    const missingItems = {};
    report.missingKeys.forEach((v) => missingItems[v.path] = v.path);

    let fileContent = '';

    if (extname(writePath) === '.js') {
      fileContent = `export default ${JSON.stringify(missingItems, null, 1)}`;
    } else {
      fileContent = JSON.stringify(missingItems, null, 1);
    }
